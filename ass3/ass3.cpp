...
void testGraph0DFS() {
    ...
    if (!g.ReadFile("graph0.txt"))
        return;
    isOK(g.NumberOfVertices(), 3, "graph number of vertices");
    isOK(g.HasVertex("A"), true, "a in graph");
...
void testGraph1() {
    ...
    if (!g.ReadFile("graph1.txt"))
        return;
    isOK(g.NumberOfVertices(), 10, "graph number of vertices");
    g.DFS("A", labelVisitor);
...
void testGraph2() {
    ...
    if (!g.ReadFile("graph2.txt"))
        return;
    isOK(g.NumberOfVertices(), 21, "graph number of vertices");
